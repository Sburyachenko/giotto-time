...
class KalmanFilter:
    ...
    def predict(self, a, p, x):
        ...
        # p_hat = mat_square(p, self.K) + np.matmul(self.R.T, self.R)
    ...
    def initial_predict(self, x):
        ...
        gain = multi_dot([p, np.transpose(self.Z), np.linalg.inv(F)])
        # a_hat = np.matmul(self.K, a)
        # p_hat = mat_square(p, self.K) + mat_square(self.Q, self.R)
        # a_hat = np.matmul(self.K, a) + multi_dot([self.K, gain, nu])
        # p_hat = multi_dot([self.K, p, (self.K - np.dot(gain, self.Z)).T])
